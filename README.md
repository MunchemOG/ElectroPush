...
### [Epsh Extreme](#epsh-extreme) reloads your code instead of installing it
...
- **Stock FTC Dashboard.** Sloth includes a drop-in replacement of Dashboard to
  make it compatible. Epsh registers your `@Config` classes with the real one
...
- **It is the command you already run.** `epsh` decides for itself whether to
  reload or install; there is no second task to remember.

**Where Sloth is better**

- **Sloth is faster.** It advertises under a second, ceiling of two. Epsh
...
- **Sloth is safe to deploy while an OpMode is running**, because it applies the
  change when the OpMode ends. Epsh Extreme reloads immediately, and what that
  does mid-OpMode has not been established. Stop the OpMode first.
- **`@Pinned` is finer grained.** Sloth pins individual classes. Epsh keeps
...
- **Sloth is an extensible runtime.** Sinister classpath scanning lets libraries
  and your own code hook into reloads. Epsh has one internal bridge and no
  public extension point.
- **Sloth is mature.** Epsh Extreme has been measured on one project, on one
...
```bash
brew install MunchemOG/homebrew-epsh/epsh
```

Or from source:

```bash
go build -o epsh && sudo mv epsh /usr/local/bin/
```
...
| Command | Description |
|---|---|
| `epsh` | Build and deploy |
| `epsh connect` | Join the robot Wi-Fi and connect adb |
| `epsh exit` | Disconnect adb and return to your Wi-Fi |
| `epsh dc` | Disconnect adb only |
| `epsh settings` | Profiles and preferences |
| `epsh slim` | Shrink the APK (`--undo` to revert) |
| `epsh hwconfig` | Pull, edit and push the robot's hardware configs |
| `epsh doctor` | Diagnose Wi-Fi, adb and project problems |
| `epsh visualiser <OpMode>` | Draw the path an auto drove, coloured by speed |
| `epsh prepare` | Cache Gradle dependencies while online |
| `epsh help` | Help |

## Settings

`epsh settings` opens a menu covering robot profiles, which network to return
to, whether to prefer USB, slimming, delta transfer, and Gradle threads. Changes
save immediately to `~/.config/epsh/config.yaml`.

**Update epsh** checks for a newer release and installs it. A Homebrew install
...
Driver Station writes. `epsh hwconfig` brings those into your project so they
...
epsh hwconfig list            what the robot and the project each have
epsh hwconfig pull            copy the robot's configs into configs/
epsh hwconfig view comp       show what is wired where
epsh hwconfig edit comp       open it in $EDITOR, check it, offer to push
epsh hwconfig diff            what changed against the robot
epsh hwconfig push comp       copy it back
```
...
If you would rather use your own editor, `epsh hwconfig edit <name>` opens
...
stop the push (`--force` overrides); anything epsh is unsure about is a
...
saved into `configs/.epsh-backup/` first, because it may have been changed on
...
pick it → Activate. Epsh says so when the file you pushed is the active one.

Reading *which* configuration is active needs privileged adb. That works on a
Control Hub; on a phone robot controller epsh says it could not tell rather
than guessing.

## Visualising an autonomous

`epsh visualiser CloseBlue` pulls a path trace off the robot and renders an HTML
page: the whole flow of the auto, every curve coloured by modelled speed, and a
duration estimate next to the measured time.

```bash
epsh visualiser CloseBlue     # newest trace for that OpMode
epsh visualiser               # newest trace on the robot
...
stack trace on each path commit and epsh maps the line number back into your
...
Colour is modelled speed, not commanded power. Epsh runs a forward/backward
...
under `/data/local/tmp/epsh`, which survives reboots, so later pushes transfer
...
**`epsh slim`** drops the native libraries for the CPU your hub does not have,
...
edits are backed up next to themselves; `epsh slim --undo` restores them.
...
`epsh settings` -> **Deploy speed** has a switch for each part of that, because
...
| Store native libraries uncompressed | stops the install extracting 20 MB+ of libraries, at the cost of a bigger APK. Applied by `epsh slim` | off |
...
Epsh Extreme reloads your OpModes onto a running robot instead of installing
...
transfer trick epsh has was within noise of that, because the cost is the
install and not the transfer. Epsh Extreme does not install:
...
Epsh Extreme is slower than that today. Its 1.09s is the part that reaches
...
- **FtcDashboard keeps working.** Dashboard finds `@Config` classes by scanning
  the APK, which reloaded code is not in. Epsh registers them from inside the
...
- **No changes to the robot controller app.** No custom classloader, no
  ...
  Epsh uses a path the SDK already supports.
...
- **Only `org.firstinspires.ftc.teamcode` is reloaded.** Everything else lives
  in the APK.
- **Hardware device drivers written in team code stay in the APK.** Epsh finds
...
- **Your team code is not in the APK while this is set up.** That is what makes
  ...
  from Android Studio gets a robot with no OpModes until epsh reloads them.
...
- **Code can compile and still not reload.** Installing or changing a library,
  or changing anything outside team code, needs a full install. Epsh detects
...
`epsh settings` -> **Epsh Extreme** -> Set up this project, then turn on
...
file to know when to rescan. Epsh compiles your team code on your laptop,
...
epsh registers your `@Config` classes with it from inside the reload.
...
Epsh needs to know which network to put you back on. How it works that out
differs by platform. `epsh doctor` shows which backend is in use.
...
macOS 13. Epsh instead reads the saved-network list, which macOS keeps in
...
connect to the robot yourself and epsh will deploy over that connection.

**Windows** reports the SSID freely, so a normal push is fine. But it keeps no
record of when each saved network was last used, so a standalone `epsh exit`
cannot tell where you came from. Set the network to return to in
`epsh settings` → Home Wi-Fi network. Note also that `netsh` cannot take a
password inline, so epsh generates a WPA2-PSK profile and imports it before
connecting.

If the network is ever guessed wrong on any platform, pin it in
`epsh settings` → Home Wi-Fi network, which always wins.
